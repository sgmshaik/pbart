...
class TreeMCMC(Tree):
    ...
    def evaluate_new_subtree(self, data, node_id_start, param, nodes_subtree, cache, settings, databc, cachebc):
        ...
        for node_id in nodes_subtree:
            ...
            if node_id in self.leaf_nodes:
                ...
                self.recompute_prob_split(data, param, settings, cache, node_id, databc, cachebc)
        if settings.debug == 1:
            try:
...
def run_mcmc_single_tree(p, settings, data, param, cache, change, mcmc_counts, cache_tmp,databc,cachebc, pmcmc=None):
    if settings.mcmc_type == 'cgm' or settings.mcmc_type == 'growprune':
        (change, step_id) = p.sample(data, settings, param, cache,databc, cachebc)
    elif settings.mcmc_type == 'pg':
